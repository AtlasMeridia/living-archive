"""Dashboard API functions — all data served from catalog.db cache tables.

No NAS access, no filesystem walks. The catalog must be populated first
via `python -m src.catalog scan` and `python -m src.catalog refresh`.
"""
