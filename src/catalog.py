...
Provides a single queryable view across photos and documents. Manifests
remain the source of truth; the catalog is a derived, rebuildable
aggregation used by discovery, batch processing, and the Immich sync.
...
CREATE INDEX IF NOT EXISTS idx_assets_slice ON assets(slice);
"""
...
def scan_directory(conn: sqlite3.Connection, directory: Path, content_type: str,
...
def main():
    ...
    if command == "stats":
        ...
        photo_count = backfill_from_manifests(conn, config.AI_LAYER_DIR, "photo")
        _log.info("Backfilled %d photo manifests", photo_count)

        doc_count = backfill_from_manifests(conn, config.DOC_AI_LAYER_DIR, "document")
